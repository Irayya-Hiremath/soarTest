  ...
  return (
    <div className="border lg:w-[445px] md:w-[445px] max-h-[276px] p-4 bg-white  rounded-lg shadow-md">
      <div className="flex  items-center space-x-4 ">
        {scrollIndex > 0 && (
          <div className=" h-[50px] cursor-pointer flex justify-center items-center w-[50px] bg-[#FFFFFF] shadow-xl rounded-full hover:bg-gray-300">
            <Icon
              onClick={handlePrev}
              style={{ height: '25px', width: '25px' }}
              icon="material-symbols-light:chevron-backward"
            />
          </div>
        )}
        ...
        {users?.slice(scrollIndex, scrollIndex + 3).map((user) => (
          ...
          >
            ...
            />
            <p
              className={`text-center text-[14px] md:text-sm lg:text-sm font-medium${
            ...
            <p
              className={`text-center text-[10px] md:text-xs lg:text-xs font-medium${
                selectedUser === user.id
                  ? 'text-[#718EBF] font-bold'
                  : 'text-[#718EBF] font-normal'
              }`}
            >
              {user.role}
            </p>
      ...
      <div className="flex justify-between content-center items-center mt-2">
        <h3 className="text-[14px] md:text-sm lg:text-sm text-[#718EBF] ">
          Write Amount
        </h3>
        <div className=" border rounded-full flex justify-between bg-[#EDF1F7]  items-center ">
          ...
          <div className=" bg-[#232323] text-white px-4 py-2 rounded-full flex items-center justify-center">
            Send
            <Icon
              style={{ height: '22px', width: '26px', padding: '4px' }}
