...
export default function DataCard({ balance, cardHolder, validThru, cardNumber, isDark })  {
    return (
      <div
      ...
      >
        <div className="flex px-6 py-4 justify-between items-center">
          ...
          <div className=" w-[35px] h-[35px] rounded-sm">
            ...
            <img
              ...
              alt='chip image'
              className={`w-[100%] h-[100%] object-cover   ${isDark?'bg-transparent':'bg-slate-300'}`}
            />
          </div>
        </div>
        ...
        <div  style={{
    background: 'linear-gradient(to right, rgba(255, 255, 255, 0.15) 15%, rgba(255, 255, 255, 0) 100%)',
  }}className= {`w-full h-[100%] bg-gradient-to-b  from-[#5B5A6F] to-[#000000] ${
    isDark?' border-none ':'border border-t-1 border-b-0 border-l-0 border-r-0'}}`}>
          <p className=" px-6  text-lg font-medium">{cardNumber}</p>
